        ...
        <div class="overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800 dark:text-gray-100">
          <div class="p-5">
            ...
            <div class="mt-6 flex justify-end space-x-3">
              <button
                type="button"
                class="inline-flex justify-center border border-transparent rounded-md bg-blue-600 px-4 py-2 text-sm text-white font-medium shadow-sm disabled:cursor-not-allowed hover:bg-blue-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                :disabled="!isConnected || !canExport || currentCommand?.status === 'running'"
                @click="handleExport"
              >
                <span v-if="currentCommand?.status === 'running'" class="mr-2">
                  <div class="i-carbon-circle-dash inline-block h-4 w-4 animate-spin" />
                </span>
                {{ currentCommand?.status === 'running' ? '导出中...' : '开始导出' }}
              </button>
            </div>
          </div>
        </div>
